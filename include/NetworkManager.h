...
public:
    ...
    void startListeningForOpponentMoves(std::function<void(int)> on_opponent_move_received);
    void stopListening();
...
private:
    ...
    bool retryOperation(std::function<bool()> operation, int maxRetries = MAX_RETRY_COUNT);
};
