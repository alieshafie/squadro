...
SquadroAI::NetworkManager::NetworkManager(const std::string& gui_ip, int my_player_gui_port,
...
bool SquadroAI::NetworkManager::sendMoveToGui(int pawn_to_move_idx) {
    ...
    if (!m_gui_client) {
        m_gui_client = std::make_unique<httplib::Client>(m_gui_ip, m_my_player_gui_port);
    }
...
void SquadroAI::NetworkManager::startListeningForOpponentMoves(std::function<void(int opponent_pawn_move_idx)> on_opponent_move_received) {
    ...
    m_listen_server->Post("/move", [this, on_opponent_move_received](const httplib::Request& req, httplib::Response& res) {
        try {
    ...
    m_server_thread = std::thread([this]() {
        std::cout << "Server started listening on " << m_my_listen_ip << ":" << m_my_listen_reply_port << std::endl;
