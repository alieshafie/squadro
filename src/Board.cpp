...
std::optional<Board::AppliedMoveInfo> Board::applyMove(
  ...
  MoveResult move_result = calculateMoveResult(mover);

  if (!move_result.is_valid) {
    return std::nullopt;
  }

  // --- Apply the pre-calculated changes to the actual board state ---
  AppliedMoveInfo move_info;
  move_info.mover_id = static_cast<uint8_t>(mover.id);
  ...
  move_info.dest_row = move_result.dest_row;
  move_info.dest_col = move_result.dest_col;
  move_info.captured_count = move_result.captured_count;
  move_info.captures = move_result.captures;

  // 1. Update grid for the mover's starting position
  cell_ref(mover.row, mover.col) = EMPTY_CELL;

  // 2. Update all captured pieces
  for (uint8_t i = 0; i < move_result.captured_count; ++i) {
    const auto& capture = move_result.captures[i];
    Piece& opponent = pieces[capture.id];

    // Determine reset location (logic duplicated from calculation, but safer)
    ...
    cell_ref(opponent.row, opponent.col) = EMPTY_CELL;
    opponent.row = reset_row;
    opponent.col = reset_col;
    cell_ref(opponent.row, opponent.col) = opponent.id;
  }

  // 3. Update the mover piece itself
  mover.row = move_result.dest_row;
  mover.col = move_result.dest_col;
  mover.status = move_result.final_status;

  // 4. Update grid for the mover's final position
  if (!mover.isFinished()) {
...
void Board::generateCaptureMoves(PlayerID player, MoveList& moves) const {
  ...
  for (int i = 1; i <= PIECES_PER_PLAYER; ++i) {
    ...
    // Fast check for a potential capture before calling the full isMoveValid
